package object cafebabe {
  // Needed for conversion of types to their name
  ...
  def typeToClassName(tpe: Class[_]) = tpe.getName.replace('.', ';')
  def typeToTypeRep(tpe: Class[_]) = {
    tpe.getName match {
      case "void" ⇒ "V"
      case "int" ⇒ "I"
      case "bool" ⇒ "Z"
      ...
      case "short" ⇒ "S"
      case "char" ⇒ "C"
      case aClass ⇒ s"L${aClass.replace('.', '/')};"
    }
  }
}
